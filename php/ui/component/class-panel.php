...
use ImageKitWordpress\REST_API;
use ImageKitWordpress\Settings;
...
class Panel extends Component {
	...
	protected function save( $struct ) {
		...
		if ( $this->setting->has_param( 'option_name' ) ) {
			...
			$struct['attributes']['class'][] = 'ik-submit';
			$button                          = $this->get_part( 'button' );
			$button['content']               = $this->setting->get_param( 'label', __( 'Save Changes', 'imagekit' ) );
			$button['attributes']['type']    = $this->type;
			$button['attributes']['name']    = 'ik_submission';
	...
	protected function header( $struct ) {
		...
		$struct['attributes']['class'][] = 'ik-' . $this->type . '-heading';
