...
use ImageKitWordpress\Settings\Storage\Storage;
...
class Settings {
	...
	public function __construct( $slug, $params = array() ) {
		...
		if ( isset( $params['storage'] ) ) {
			...
			if ( class_exists( 'ImageKitWordpress\\Settings\\Storage\\' . $params['storage'] ) ) {
				$params['storage'] = 'ImageKitWordpress\\Settings\\Storage\\' . $params['storage'];
			}
		} else {
			// Default.
			$params['storage'] = 'ImageKitWordpress\\Settings\\Storage\\Options';
		}
	...
	public function get_value( ...$slugs ) {
		...
		foreach ( $slugs as $slug ) {
			 ...
			 *
			 * @hook imagekit_setting_get_value
			 *
			 * @param $value {mixed} The setting value.
			 * @param $slug  {string}  The setting slug.
			 */
			$return[ $slug ] = apply_filters( 'imagekit_setting_get_value', $value, $slug );
		}
	...
	public function set_pending( $slug, $new_value, $current_value = null ) {
		 ...
		 *
		 * @hook   imagekit_settings_save_setting_{$slug}
		 * @hook   imagekit_settings_save_setting
		 * @since  2.7.6
		 *
		 ...
		 * @return {mixed}
		 */
		$new_value = apply_filters( "imagekit_settings_save_setting_{$slug}", $new_value, $current_value, $setting );
		$new_value = apply_filters( 'imagekit_settings_save_setting', $new_value, $current_value, $setting );
