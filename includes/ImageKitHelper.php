...
class ImageKitHelper {
	...
	static public function url_query( $params = array(), $skip_empty = false,
		...
		foreach ( (array) $params as $key => $value ) {
			...
			if ( is_array( $value ) ) {
				if ( count( $value ) ) {
					$str .= ( $str != '' ? '&' : '' ) .
						ImageKitHelper::url_query( $value, $skip_empty, $key );
				}
			} else {
				$name = '';
	...
	static public function is_wpmu() {
		...
		if ( $wpmu === null ) {
			$wpmu = ( file_exists( ABSPATH . 'wpmu-settings.php' ) ||
				...
				defined( 'SUNRISE' ) ||
				ImageKitHelper::is_wpmu_subdomain() );
		}
	...
	static public function is_https() {
		...
		case ( isset( $_SERVER['HTTPS'] ) &&
				ImageKitHelper::to_boolean( $_SERVER['HTTPS'] ) ):
	...
	static public function cache_blog_minify_dir() {
		...
		if ( ImageKitHelper::is_wpmu() &&
			ImageKitHelper::is_using_master_config() &&
			! Dispatcher::config()->get_boolean( 'minify.auto' ) )
			$path = ImageKitHelper::cache_blog_dir( 'minify', 0 );
		else
			$path = ImageKitHelper::cache_blog_dir( 'minify' );
	...
	static public function document_root() {
		...
		if ( !empty( $_SERVER['SCRIPT_FILENAME'] ) &&
			...
			$script_filename = ImageKitHelper::normalize_path(
				$_SERVER['SCRIPT_FILENAME'] );
			$php_self = ImageKitHelper::normalize_path(
				$_SERVER['PHP_SELF'] );
		...
		if ( !empty( $_SERVER['PATH_TRANSLATED'] ) ) {
			$document_root = substr(
				ImageKitHelper::normalize_path( $_SERVER['PATH_TRANSLATED'] ),
				0,
				-strlen( ImageKitHelper::normalize_path( $_SERVER['PHP_SELF'] ) ) );
	...
	static public function url_relative_to_full( $relative_url ) {
		...
		if (version_compare(PHP_VERSION, '5.4.7') < 0) {
			if ( substr( $relative_url, 0, 2) == '//' ) {
				$relative_url =
					( ImageKitHelper::is_https() ? 'https' : 'http' ) .
					':' . $relative_url;
			}
		}
	...
	static public function w3tc_edition( $config = null ) {
		if ( ImageKitHelper::is_w3tc_enterprise( $config ) )
			return 'enterprise';
		if ( ImageKitHelper::is_w3tc_pro( $config ) &&  ImageKitHelper::is_w3tc_pro_dev() )
			return 'pro development';
		if ( ImageKitHelper::is_w3tc_pro( $config ) )
			return 'pro';
		return 'community';
	}
	...
	static public function stripslashes( $var ) {
		if ( is_string( $var ) ) {
			...
			$var = array_map( array( '\W3TC\ImageKitHelper', 'stripslashes' ), $var );
		}
	...
	static function replace_folder_placeholders_to_uri( $file ) {
		...
		if ( empty( $content_uri ) ) {
			...
			if ( isset( $upload_dir['baseurl'] ) )
				$uploads_uri = ImageKitHelper::url_to_uri( $upload_dir['baseurl'] );
			else
				$uploads_uri = '';
		}
