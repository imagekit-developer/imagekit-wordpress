...
class Wizard {
	constructor(context) {
		...
		this.step2DefaultErrorMessage = this.step2ErrorEl ? this.step2ErrorEl.textContent : '';
		this.step2DebounceTimer = null;
		this.step2DebounceMs = 600;
		this.step2ValidatedPayloadKey = null;
		this.step2FieldErrorEls = {
			urlEndpoint: context.querySelector('[data-wizard-field-error="urlEndpoint"]'),
			publicKey: context.querySelector('[data-wizard-field-error="publicKey"]'),
			privateKey: context.querySelector('[data-wizard-field-error="privateKey"]'),
		};
	...
	_bind() {
		...
		const step2PublicKeyInput = this.context.querySelector('#ik_wizard_public_key');
		const step2PrivateKeyInput = this.context.querySelector('#ik_wizard_private_key');
		...
		if (this.buttonNext) {
			this.buttonNext.addEventListener('click', async (ev) => {
				...
				if (this.currentStep === 2) {
					...
					if (this.step2ValidatedPayloadKey && this.step2ValidatedPayloadKey === key) {
						this.goTo(3);
						return;
					}
	...
	async _testConnectionDebounced(expectedKey) {
		...
		try {
			...
			this.step2ValidatedPayloadKey = null;
			this._setStep2FieldErrors(result && result.fieldErrors ? result.fieldErrors : null);
			const message = result && result.message ? result.message : 'Unable to verify connection. Please check your settings.';
	...
	_getWizardPayload() {
		const urlEndpointInput = this.context.querySelector('#ik_wizard_url_endpoint');
		const publicKeyInput = this.context.querySelector('#ik_wizard_public_key');
		const privateKeyInput = this.context.querySelector('#ik_wizard_private_key');
	...
	async _postTestConnection() {
		const data = window.ikData && window.ikData.wizard ? window.ikData.wizard : null;
